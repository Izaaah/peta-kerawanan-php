...
@section('content')
    <div class="max-w-7xl mx-auto py-8 px-1 pt-1 pb-2">
        ...
        @endif
        <form method="GET" action="{{ route('operator.data.medsos.index') }}" class="mb-4 flex gap-2">
        ...
        <div class="bg-white rounded shadow p-6">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($medsosList as $i => $medsos)
                        <tr>
                            <td class="px-4 py-2">{{ $medsosList->firstItem() + $i }}</td>
                            ...
                            <td class="px-4 py-2 flex gap-2">
                                <a href="{{ route('operator.data.medsos.show', $medsos->id) }}"
                                    class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-xs">Lihat</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-2 text-center text-gray-500">Belum ada data akun medsos.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
