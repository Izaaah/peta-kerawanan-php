    ...
    <div class="mx-auto px-4 py-3">
        <div class="flex justify-between items-center mb-3">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.data.penginapan.create') }}"
        ...
        @endif
        <form method="GET" action="{{ route('admin.data.penginapan.index') }}" class="mb-4 flex gap-2 form-responsive">
                    ...
                    @forelse($penginapanList as $index => $penginapan)
                        <tr>
                            <td class="px-4 py-2 text-center">
                                {{ ($penginapanList->currentPage() - 1) * $penginapanList->perPage() + $loop->iteration }}
                            </td>
                            ...
                            <td class="px-4 py-2">
                                @php
                                    $isJatim = ($penginapan->provinsi ?? '') === 'Jawa Timur';
                                    $kab = $isJatim
                                        ? $penginapan->kabupaten ?? '-'
                                        : $penginapan->kabupaten_lain ?? '-';
                                    $kec = $isJatim
                                        ? $penginapan->kecamatan ?? '-'
                                        : $penginapan->kecamatan_lain ?? '-';
                                    $kel = $isJatim
                                        ? $penginapan->kelurahan ?? '-'
                                        : $penginapan->kelurahan_lain ?? '-';
                                @endphp
                                {{ $kab }}, {{ $kec }}, {{ $kel }}
                            </td>
                            <td class="px-4 py-2">{{ $penginapan->lokasi }}</td>
                            <td class="px-1 py-2 flex gap-1 lg:gap-2 justify-center">
                                <a href="{{ route('admin.data.penginapan.show', $penginapan->id) }}"
                                ...
                                <a href="{{ route('admin.data.penginapan.edit', $penginapan->id) }}"
                                ...
                                <form action="{{ route('admin.data.penginapan.destroy', $penginapan->id) }}" method="POST"
