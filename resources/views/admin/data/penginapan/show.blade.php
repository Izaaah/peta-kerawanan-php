                    ...
                    <div class="bg-gray-50 p-4 rounded space-y-2">
                        <div>
                            @php
                                ...
                                $kab = $isJatim ? $penginapan->kabupaten ?? '-' : $penginapan->kabupaten_lain ?? '-';
                                $kec = $isJatim ? $penginapan->kecamatan ?? '-' : $penginapan->kecamatan_lain ?? '-';
                                $kel = $isJatim ? $penginapan->kelurahan ?? '-' : $penginapan->kelurahan_lain ?? '-';
                            @endphp
                            <p class="text-gray-900"><span class="font-medium">Alamat:
                                </span>{{ $kab }}, Kecamatan {{ $kec }}, Kelurahan/desa
                                {{ $kel }}
                            </p>
                        </div>
                        <p class="text-gray-900"><span class="font-medium">Alamat Lengkap:
                            </span>{{ $penginapan->lokasi }}</p>
                    </div>
                </div>
            </div>
