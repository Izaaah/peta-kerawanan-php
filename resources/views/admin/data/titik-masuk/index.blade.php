...
@section('content')
    ...
    <div class="mx-auto px-4 py-3">
        ...
        <form method="GET" action="{{ route('admin.data.titik-masuk.index') }}" class="mb-4 flex gap-2 form-responsive">
            ...
            <button type="submit"
                class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 text-sm lg:text-base">Search</button>
        </form>
        ...
        <div class="bg-white rounded shadow p-6">
            <div class="table-responsive">\n <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    @if ($titikMasuk->is_multi_segment && $titikMasuk->waypoints)
                                        ...
                                        @endif
                                    @else
                                        <div class="font-medium text-gray-900">{{ $titikMasuk->start_location ?? '-' }}
                                        </div>
                                    ...
                                    @else
                                        @if ($titikMasuk->transport_type)
                                            <span class="inline-flex items-center">
                                                ...
                                                <span
                                                    class="ml-1 text-xs">{{ ucfirst($titikMasuk->transport_type) }}</span>
                                            </span>
                                        @else
                                            <span class="text-gray-400">-</span>
                                        @endif
                                ...
                                <td class="px-4 py-2">
                                    @if ($titikMasuk->distance_km)
                                        <span
                                            class="text-sm text-gray-600">{{ number_format($titikMasuk->distance_km, 2) }}
                                            km</span>
                                    @else
                                        <span class="text-gray-400">-</span>
                                    @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-2 text-center text-gray-500">Belum ada data titik masuk.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
