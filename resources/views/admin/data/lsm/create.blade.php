...
@section('content')
            ...
            <div class="order-2 lg:order-1 lg:col-span-2">
                ...
                <form action="{{ route('admin.data.lsm.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="bg-white shadow rounded p-6 space-y-4">
                        ...
                        <div>
                            <label for="nama_lsm" class="block text-sm font-medium text-gray-700">Nama LSM <span
                                    class="text-red-500">*</span></label>
                            <input type="text" id="nama_lsm" name="nama_lsm" value="{{ old('nama_lsm') }}"
                                ...
                                <select id="kabupaten" name="kabupaten"
                                    ...
                                    @isset($kabupatenList)
                                        @foreach ($kabupatenList as $kab)
                                            <option value="{{ $kab }}"
                                                {{ old('kabupaten') == $kab ? 'selected' : '' }}>{{ $kab }}</option>
                                        @endforeach
                                    @endisset
                                </select>
                            </div>
                        ...
                        <div>
                            <label for="ketua_lsm" class="block text-sm font-medium text-gray-700">Ketua LSM <span
                                    class="text-red-500">*</span></label>
                            <input type="text" id="ketua_lsm" name="ketua_lsm" value="{{ old('ketua_lsm') }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            @error('ketua_lsm')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="no_hp_ketua" class="block text-sm font-medium text-gray-700">No. HP Ketua <span
                                    class="text-red-500">*</span></label>
                            <input type="text" id="no_hp_ketua" name="no_hp_ketua" value="{{ old('no_hp_ketua') }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            @error('no_hp_ketua')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="bg-white shadow rounded p-6">
                        ...
                        <div class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('admin.data.lsm.template') }}"
                        ...
                        <form action="{{ route('admin.data.lsm.import') }}" method="POST" enctype="multipart/form-data"
    ...
    <script>
        ...
        kabupatenSelect && kabupatenSelect.addEventListener('change', function() {
        ...
        kecamatanSelect && kecamatanSelect.addEventListener('change', function() {
