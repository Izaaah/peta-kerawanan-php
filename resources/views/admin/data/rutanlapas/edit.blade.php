                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="nik" value="{{ old('nik', $rutanlapas->nik) }}"
                                class="w-full border-gray-300 rounded px-3 py-2" readonly>
                        </div>
                        <div>
                            <label class="block font-semibold mb-1">Nama Napi</label>
                            <input type="text" name="nama_napi" value="{{ old('nama_napi', $rutanlapas->nama_napi) }}"
                                class="w-full border-gray-300 rounded px-3 py-2" readonly>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="jenis_napi" class="w-full border-gray-300 rounded px-3 py-2" required>
                                ...
                                @foreach ($jenisNapiOptions as $jenis)
                                    <option value="{{ $jenis }}"
                                        {{ old('jenis_napi', $rutanlapas->jenis_napi) == $jenis ? 'selected' : '' }}>
                                        {{ $jenis }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="lapas" value="{{ old('lapas', $rutanlapas->lapas) }}"
                                class="w-full border-gray-300 rounded px-3 py-2" required>
                        </div>

                        <div>
                            <label class="block font-semibold mb-1">Peran dalam Jaringan</label>
                            <select name="peran_dalam_jaringan" class="w-full border-gray-300 rounded px-3 py-2">
                                <option value="">-- Pilih Peran --</option>
                                @php($peran = old('peran_dalam_jaringan', $rutanlapas->peran_dalam_jaringan))
                                <option value="Penyalahguna" {{ $peran == 'Penyalahguna' ? 'selected' : '' }}>Penyalahguna
                                </option>
                                <option value="Pecandu" {{ $peran == 'Pecandu' ? 'selected' : '' }}>Pecandu</option>
                                <option value="Kurir" {{ $peran == 'Kurir' ? 'selected' : '' }}>Kurir</option>
                                <option value="Bandar" {{ $peran == 'Bandar' ? 'selected' : '' }}>Bandar</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="lokasi_lapas" rows="3" class="w-full border-gray-300 rounded px-3 py-2 resize-none" required
                                placeholder="Masukkan lokasi lapas...">{{ old('lokasi_lapas', $rutanlapas->lokasi_lapas) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status_proses" class="w-full border-gray-300 rounded px-3 py-1" required>
                                ...
                                @foreach ($statusProsesOptions as $status)
                                    <option value="{{ $status }}"
                                        {{ old('status_proses', $rutanlapas->status_proses) == $status ? 'selected' : '' }}>
                                        {{ $status }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" rows="3" class="w-full border-gray-300 rounded px-3 py-1 resize-none"
                                placeholder="Masukkan keterangan...">{{ old('keterangan', $rutanlapas->keterangan) }}</textarea>
                        </div>
                    </div>
                </div>
