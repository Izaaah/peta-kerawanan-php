...
<div class="container mx-auto px-1 pt-1 pb-2 max-w-7xl">
    ...
    <form method="GET" action="{{ route('admin.data.objekvital.index') }}" class="mb-4 flex gap-2">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-4 py-2 flex gap-2">
                        <a href="{{ route('admin.data.objekvital.show', $objekVital->id) }}" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-xs">Lihat</a>
                        <a href="{{ route('admin.data.objekvital.edit', $objekVital->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 text-xs">Edit</a>
                        <form action="{{ route('admin.data.objekvital.destroy', $objekVital->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')" class="inline">
                            @csrf
                            @method('DELETE')
