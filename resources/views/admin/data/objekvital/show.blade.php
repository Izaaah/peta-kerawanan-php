...
<div class="max-w-2xl mx-auto py-8">
    <div class="bg-white rounded shadow p-6">
        ...
        <div class="flex gap-2 mt-6">
            <a href="{{ route('admin.data.objekvital.edit', $objekVital->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">Edit</a>
            <form action="{{ route('admin.data.objekvital.destroy', $objekVital->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Hapus</button>
            </form>
            <a href="{{ route('admin.data.objekvital.index') }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">Kembali</a>
        </div>
    </div>
</div>
@endsection
