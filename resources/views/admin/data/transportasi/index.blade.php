...
@section('content')
    ...
    <div class="mx-auto px-4 py-3">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Jasa Transportasi</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Tempat Transportasi</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('admin.data.transportasi.create') }}"
        ...
        <form method="GET" action="{{ route('admin.data.transportasi.index') }}" class="mb-4 flex gap-2 form-responsive">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari jenis, nama pihak, alamat..."
            ...
            <div class="table-responsive">\n                <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Jenis Jasa Transportasi
                        </th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Jasa Transportasi
                        </th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Alamat</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Kepala/Manajer</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($transportasiList as $i => $transportasi)
                        <tr>
                            <td class="px-4 py-2 text-center">{{ $transportasiList->firstItem() + $i }}</td>
                            <td class="px-4 py-2">
                                <span
                            ...
                            <td class="px-4 py-2">
                                ...
                                @if ($transportasi->alamat)
                                    <div class="text-xs text-gray-500">{{ $transportasi->alamat }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-2">{{ $transportasi->nama_manager ?? '-' }}</td>
                            <td class="px-4 py-2">{{ $transportasi->jabatan ?? '-' }}</td>
                            <td class="px-4 py-2">{{ $transportasi->no_hp }}</td>
                            <td class="px-1 py-2 flex gap-1 lg:gap-2 justify-center">
                                <a href="{{ route('admin.data.transportasi.show', $transportasi->id) }}"
                                ...
                                <form action="{{ route('admin.data.transportasi.destroy', $transportasi->id) }}"
                                    method="POST" class="inline-block"
                                    onsubmit="return confirm('Yakin ingin menghapus user ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="text-red-600 hover:text-red-900 flex items-center border border-red-600 rounded-md px-1 py-1 text-xs lg:text-sm">
                                        <svg class="w-3 h-3 lg:w-4 lg:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
