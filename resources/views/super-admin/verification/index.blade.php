            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700">
                        <p>
                            • <strong>Approve:</strong> Data akan diperbarui dan Anda akan diarahkan ke halaman edit<br>
                            • <strong>Reject:</strong> Data akan ditolak dan tidak disimpan ke sistem<br>
                            • <strong>Data Baru:</strong> Data yang belum ada di sistem sebelumnya<br>
                            • <strong>Update:</strong> Data yang sudah ada akan diperbarui dengan informasi baru
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($verifications as $verification)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 text-sm text-center">{{ $verification->id }}</td>
                                <td class="px-4 py-2">
                                    ...
                                    @if ($verification->data_id == 0)
                                        <span
                                            class="ml-1 inline-block bg-green-100 text-green-800 text-xs px-1 py-0.5 rounded">Baru</span>
                                    @else
                                        <span
                                            class="ml-1 inline-block bg-orange-100 text-orange-800 text-xs px-1 py-0.5 rounded">Update</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <div class="flex gap-2 justify-center">
                                        ...
                                        <form action="{{ route('super-admin.verification.approve', $verification->id) }}"
                                            method="POST"
                                            onsubmit="return confirm('{{ $verification->data_id == 0 ? 'Yakin approve data baru ini?' : 'Yakin ingin memperbarui data dan diarahkan ke halaman edit?' }}')"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="inline-flex items-center px-2 py-1 text-xs text-green-600 border border-green-600 rounded hover:bg-green-50"
                                                title="{{ $verification->data_id == 0 ? 'Approve Data Baru' : 'Approve & Edit Data' }}">
                                                <i class="fas fa-check"></i>
                                            </button>
                                        </form>
                                        <form action="{{ route('super-admin.verification.reject', $verification->id) }}"
                                            method="POST" onsubmit="return confirm('Yakin reject data ini?')"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
