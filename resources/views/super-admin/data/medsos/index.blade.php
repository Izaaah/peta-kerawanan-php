...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Akun Media Sosial</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai akun media sosial transaksi narkotika
                </p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.medsos.index') }}"
            class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari media sosial, nama akun..."
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Media Sosial
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Jenis Akun
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Akun
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama
                                Individu
                                (Personal)
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Link Akun
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($medsosList as $index => $medsos)
                            <tr>
                                <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">
                                    {{ ($medsosList->currentPage() - 1) * $medsosList->perPage() + $loop->iteration }}
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $medsos->nama_media_sosial ?? '-' }}
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">
                                    @if ($medsos->jenis_akun)
                                        <span
                                            class="inline-flex px-2 py-1 text-xs font-semibold rounded-full
                                            {{ $medsos->jenis_akun === 'personal' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                                            {{ ucfirst($medsos->jenis_akun) }}
                                        </span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $medsos->nama_akun ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">
                                    @if ($medsos->jenis_akun === 'personal' && $medsos->individu)
                                        <div class="font-semibold">{{ $medsos->individu->nama ?? '-' }}</div>
                                        <div class="text-gray-600">NIK: {{ $medsos->individu->nik ?? '-' }}</div>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">
                                    @if ($medsos->link_akun)
                                        <a href="{{ $medsos->link_akun }}" target="_blank"
                                            class="text-blue-600 hover:text-blue-800 underline">
                                            {{ Str::limit($medsos->link_akun, 30) }}
                                        </a>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $medsos->user->name ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                        <a href="{{ route('super-admin.data.medsos.show', $medsos->id) }}"
                                        ...
                                        <a href="{{ route('super-admin.data.medsos.edit', $medsos->id) }}"
                                        ...
                                        <form action="{{ route('super-admin.data.medsos.destroy', $medsos->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-gray-500 text-sm lg:text-base">Belum
                                    ada data akun medsos.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
