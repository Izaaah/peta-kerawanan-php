...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Lembaga Rehabilitasi</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Lembaga Rehabilitasi</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.lrehab.index') }}" class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($lrehabList as $i => $lrehab)
                            <tr>
                                ...
                                <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">{{ $lrehabList->firstItem() + $i }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lrehab->nama }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lrehab->jenis }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lrehab->user->name ?? '-' }}</td>
                            <td class="px-2 lg:px-4 py-2">
                                <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                    <a href="{{ route('super-admin.data.lrehab.show', $lrehab->id) }}"
                                    ...
                                    <a href="{{ route('super-admin.data.lrehab.edit', $lrehab->id) }}"
                                    ...
                                    <form action="{{ route('super-admin.data.lrehab.destroy', $lrehab->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-gray-500 text-sm lg:text-base">Belum ada data Lembaga
                                Rehabilitasi.</td>
                        </tr>
                    @endforelse
                </tbody>
