    ...
    <div class="bg-white rounded shadow p-6">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-4 py-2 text-center">{{ ($vapeList->currentPage() - 1) * $vapeList->perPage() + $index + 1 }}</td>
                    <td class="px-4 py-2">{{ $vape->nama_toko }}</td>
                    <td class="px-4 py-2">{{ $vape->pemilik }}</td>
                    <td class="px-4 py-2">{{ $vape->no_hp }}</td>
                    <td class="px-4 py-2">{{ $vape->user->name ?? '-' }}</td>
                    <td class="px-1 py-2 flex gap-2 justify-center">
                        ...
                        <form action="{{ route('super-admin.data.vape.destroy', $vape->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus user ini?');">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-2 text-center text-gray-500">Belum ada data penjual vape.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
