...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Penjual Vape</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Penjual Vape</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.vape.index') }}"
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Toko
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Pemilik</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($vapeList as $index => $vape)
                            <tr>
                                ...
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $vape->nama_toko }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $vape->pemilik }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $vape->no_hp }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $vape->user->name ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                        <a href="{{ route('super-admin.data.vape.show', $vape->id) }}"
                                        ...
                                        <a href="{{ route('super-admin.data.vape.edit', $vape->id) }}"
                                        ...
                                        <form action="{{ route('super-admin.data.vape.destroy', $vape->id) }}"
