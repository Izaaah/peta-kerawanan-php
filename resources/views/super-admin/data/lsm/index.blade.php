...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar LSM Narkotika</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai LSM</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.lsm.index') }}"
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama LSM
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Ketua</th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">No. HP Ketua
                            </th>
                        ...
                        @forelse($lsmList as $index => $lsm)
                            <tr>
                                <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">
                                    {{ ($lsmList->currentPage() - 1) * $lsmList->perPage() + $loop->iteration }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm capitalize">{{ $lsm->nama_lsm }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm capitalize">{{ $lsm->ketua_lsm ?? '-' }}
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lsm->no_hp_ketua }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lsm->alamat }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $lsm->user->name ?? 'N/A' }}</td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                        <a href="{{ route('super-admin.data.lsm.show', $lsm->id) }}"
                                        ...
                                        <a href="{{ route('super-admin.data.lsm.edit', $lsm->id) }}"
                                        ...
                                        <form action="{{ route('super-admin.data.lsm.destroy', $lsm->id) }}" method="POST"
