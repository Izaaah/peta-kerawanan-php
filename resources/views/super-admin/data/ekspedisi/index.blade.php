...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Ekspedisi</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Ekspedisi</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.ekspedisi.index') }}" class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            ...
            <div class="table-responsive overflow-x-auto">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Manager</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($ekspedisiList as $i => $ekspedisi)
                        <tr>
                            <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">{{ $ekspedisiList->firstItem() + $i }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->nama }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->manager }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->alamat }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->no_hp }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->jenis }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $ekspedisi->user->name ?? '-' }}</td>
                            <td class="px-2 lg:px-4 py-2">
                                <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                <a href="{{ route('super-admin.data.ekspedisi.show', $ekspedisi->id) }}"
                                ...
                                <a href="{{ route('super-admin.data.ekspedisi.edit', $ekspedisi->id) }}"
                                ...
                                <form action="{{ route('super-admin.data.ekspedisi.destroy', $ekspedisi->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-gray-500 text-sm lg:text-base">Belum ada data ekspedisi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
