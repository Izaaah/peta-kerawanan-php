...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        ...
        <form method="GET" action="{{ route('super-admin.data.titik-masuk.index') }}"
            class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            <input type="text" name="q" value="{{ request('q') }}"
                placeholder="Cari route name, start location, transport type..."
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Route Name
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Start
                                Location</th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Transport
                                Type</th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Distance
                            </th>
                            <th class="px-2 lg:px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">Status
                            </th>
                        ...
                        @forelse($titikMasukList as $i => $titikMasuk)
                            <tr>
                                <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">
                                    {{ $titikMasukList->firstItem() + $i }}</td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="font-medium text-gray-900 text-xs lg:text-sm">
                                        {{ $titikMasuk->route_name ?? '-' }}</div>
                                </td>
                                <td class="px-2 lg:px-4 py-2">
                                    ...
                                    @if ($titikMasuk->is_multi_segment && $titikMasuk->waypoints)
                                        @php
                                            $waypoints = is_string($titikMasuk->waypoints)
                                                ? json_decode($titikMasuk->waypoints, true)
                                                : $titikMasuk->waypoints;
                                            $transportTypes = array_filter(
                                                array_column($waypoints, 'transport_to_next'),
                                            );
                                            ...
                                            <div class="flex flex-wrap gap-1">
                                                @foreach ($uniqueTransports as $transport)
                                                    <span class="inline-flex items-center text-xs">
                                                        ...
                                                        <span
                                                            class="ml-1 hidden lg:inline">{{ ucfirst($transport) }}</span>
                                                    </span>
                                                @endforeach
                                            </div>
                                        @else
                                    ...
                                    @else
                                        @if ($titikMasuk->transport_type)
                                            <span class="inline-flex items-center">
                                                ...
                                                <span
                                                    class="ml-1 text-xs hidden lg:inline">{{ ucfirst($titikMasuk->transport_type) }}</span>
                                            </span>
                                        @else
                                            <span class="text-gray-400 text-xs">-</span>
                                        @endif
                                ...
                                <td class="px-2 lg:px-4 py-2">
                                    @if ($titikMasuk->distance_km)
                                        <span
                                            class="text-xs lg:text-sm text-gray-600">{{ number_format($titikMasuk->distance_km, 2) }}
                                            km</span>
                                    @else
                                        <span class="text-gray-400 text-xs">-</span>
                                    @endif
