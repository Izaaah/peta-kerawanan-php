	...
	<div class="bg-white rounded shadow p-6">
		...
		<form action="{{ route('super-admin.data.titik-masuk.update', $titikMasuk->id) }}" method="POST">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="mb-4">
					...
					<select name="jenis_transportasi" id="jenis_transportasi" class="w-full border-gray-300 rounded px-3 py-2" required>
						...
						<option value="Darat" {{ old('jenis_transportasi', $titikMasuk->jenis_transportasi) === 'Darat' ? 'selected' : '' }}>Darat</option>
						<option value="Laut" {{ old('jenis_transportasi', $titikMasuk->jenis_transportasi) === 'Laut' ? 'selected' : '' }}>Laut</option>
						<option value="Udara" {{ old('jenis_transportasi', $titikMasuk->jenis_transportasi) === 'Udara' ? 'selected' : '' }}>Udara</option>
					</select>
				...
				<div class="mb-4">
					...
					<input type="text" name="nama_tempat" id="nama_tempat" value="{{ old('nama_tempat', $titikMasuk->nama_tempat) }}" class="w-full border-gray-300 rounded px-3 py-2" required>
				...
				<div class="mb-4">
					...
					<select name="provinsi" id="provinsi" class="w-full border-gray-300 rounded px-3 py-2" required>
						<option value="Jawa Timur" {{ old('provinsi', $titikMasuk->provinsi) === 'Jawa Timur' ? 'selected' : '' }}>Jawa Timur</option>
						<option value="lainnya" {{ old('provinsi', $titikMasuk->provinsi) !== 'Jawa Timur' ? 'selected' : '' }}>Lainnya</option>
					</select>
				...
				<div id="wilayah-jatim" class="md:col-span-2">
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							<label for="kabupaten" class="block font-semibold mb-1">Kabupaten</label>
							<select name="kabupaten" id="kabupaten" class="w-full border-gray-300 rounded px-3 py-2" required data-selected="{{ old('kabupaten', $titikMasuk->kabupaten) }}">
						...
						<div>
							...
							<select name="kecamatan" id="kecamatan" class="w-full border-gray-300 rounded px-3 py-2" required data-selected="{{ old('kecamatan', $titikMasuk->kecamatan) }}">
						...
						<div>
							...
							<select name="kelurahan" id="kelurahan" class="w-full border-gray-300 rounded px-3 py-2" required data-selected="{{ old('kelurahan', $titikMasuk->kelurahan) }}">
				...
				<div id="wilayah-lainnya" class="md:col-span-2 hidden">
					<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
						<div>
							...
							<input type="text" name="provinsi_lain" id="provinsi_lain" value="{{ old('provinsi', $titikMasuk->provinsi) !== 'Jawa Timur' ? old('provinsi', $titikMasuk->provinsi) : '' }}" class="w-full border-gray-300 rounded px-3 py-2" placeholder="Provinsi">
						</div>
						<div>
							<label class="block font-semibold mb-1">Kabupaten</label>
							<input type="text" name="kabupaten_lain" value="{{ old('kabupaten', $titikMasuk->provinsi) !== 'Jawa Timur' ? old('kabupaten', $titikMasuk->kabupaten) : '' }}" class="w-full border-gray-300 rounded px-3 py-2" placeholder="Kabupaten">
						</div>
						<div>
							<label class="block font-semibold mb-1">Kecamatan</label>
							<input type="text" name="kecamatan_lain" value="{{ old('provinsi', $titikMasuk->provinsi) !== 'Jawa Timur' ? old('kecamatan', $titikMasuk->kecamatan) : '' }}" class="w-full border-gray-300 rounded px-3 py-2" placeholder="Kecamatan">
						</div>
						<div>
							<label class="block font-semibold mb-1">Kelurahan/Desa</label>
							<input type="text" name="kelurahan_lain" value="{{ old('provinsi', $titikMasuk->provinsi) !== 'Jawa Timur' ? old('kelurahan', $titikMasuk->kelurahan) : '' }}" class="w-full border-gray-300 rounded px-3 py-2" placeholder="Kelurahan/Desa">
						</div>
					</div>
				</div>
			</div>
			...
			<div class="flex gap-2 mt-6">
				...
				<a href="{{ route('super-admin.data.titik-masuk.index') }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">Batal</a>
			</div>
		</form>
	</div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function fetchKecamatanList(kabupaten) {
		...
		return fetch(`/super-admin/api/kecamatan-list?kabupaten=${encodeURIComponent(kabupaten)}`)
			.then(r => r.json())
			.then(data => {
				data.forEach(kec => {
					const opt = document.createElement('option');
