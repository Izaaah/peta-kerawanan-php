...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Jaringan Rutan/Lapas</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Jaringan Rutan/Lapas</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.rutanlapas.index') }}" class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama napi, lapas, status..."
            ...
            <div class="table-responsive overflow-x-auto">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Napi</th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Jenis Napi</th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Lapas</th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Lokasi Lapas</th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Peran Dalam Jaringan
                        </th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Status Proses</th>
                        <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Keterangan</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($rutanlapasList as $i => $rutanlapas)
                        <tr>
                            <td class="px-2 lg:px-4 py-2 text-center text-xs lg:text-sm">{{ $rutanlapasList->firstItem() + $i }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->nama_napi }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->jenis_napi }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->lapas }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->lokasi_lapas }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->peran_dalam_jaringan }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->status_proses }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->keterangan }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $rutanlapas->user->name ?? '-' }}</td>
                            <td class="px-2 lg:px-4 py-2">
                                <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                    <a href="{{ route('super-admin.data.rutanlapas.show', $rutanlapas->id) }}"
                                    ...
                                    <a href="{{ route('super-admin.data.rutanlapas.edit', $rutanlapas->id) }}"
                                    ...
                                    <form action="{{ route('super-admin.data.rutanlapas.destroy', $rutanlapas->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-8 text-center text-gray-500 text-sm lg:text-base">Belum ada data jaringan rutan/lapas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
