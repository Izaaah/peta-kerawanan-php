...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Perusahaan Farmasi/Prekursor</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Perusahaan Farmasi/Prekursor</p>
            </div>
        ...
        <form method="GET" action="{{ route('super-admin.data.farmasi.index') }}" class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            ...
            <div class="table-responsive overflow-x-auto">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($farmasiList as $i => $farmasi)
                        <tr>
                            <td class="px-2 lg:px-4 py-2 text-center">{{ $farmasiList->firstItem() + $i }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $farmasi->jenis }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $farmasi->nama }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $farmasi->manager }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $farmasi->no_hp }}</td>
                            <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $farmasi->user->name ?? '-' }}</td>
                            <td class="px-2 lg:px-4 py-2">
                                <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                <a href="{{ route('super-admin.data.farmasi.show', $farmasi->id) }}"
                                ...
                                <a href="{{ route('super-admin.data.farmasi.edit', $farmasi->id) }}"
                                ...
                                <form action="{{ route('super-admin.data.farmasi.destroy', $farmasi->id) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-gray-500 text-sm lg:text-base">Belum ada data.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
