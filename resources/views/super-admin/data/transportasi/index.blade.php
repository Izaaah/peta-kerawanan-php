...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        ...
        <form method="GET" action="{{ route('super-admin.data.transportasi.index') }}"
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">Jenis
                                Transportasi</th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Pihak
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($transportasiList as $i => $transportasi)
                           ...
                           @else bg-purple-100 text-purple-800 @endif">
                                ...
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $transportasi->nama_pihak }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $transportasi->posisi ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $transportasi->lokasi ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $transportasi->no_hp }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $transportasi->user->name ?? '-' }}
                                </td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                        <a href="{{ route('super-admin.data.transportasi.show', $transportasi->id) }}"
                                        ...
                                        <a href="{{ route('super-admin.data.transportasi.edit', $transportasi->id) }}"
                                        ...
                                        <form
                                            action="{{ route('super-admin.data.transportasi.destroy', $transportasi->id) }}"
