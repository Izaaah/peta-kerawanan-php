...
@section('content')
    ...
    <div class="mx-auto px-2 lg:px-4 py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-800">Daftar Penginapan</h1>
                <p class="text-xs lg:text-sm text-gray-500">Informasi lengkap mengenai Penginapan</p>
            </div>
        ...
        @endif
        <form method="GET" action="{{ route('super-admin.data.penginapan.index') }}"
            class="mb-4 flex flex-col sm:flex-row gap-2 form-responsive">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama, jenis, pengelola..."
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Jenis</th>
                            <th class="px-2 lg:px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Pengelola
                            </th>
                        ...
                        @forelse($penginapanList as $index => $penginapan)
                            <tr>
                                <td class="px-2 lg:px-4 py-2 text-center">
                                    {{ ($penginapanList->currentPage() - 1) * $penginapanList->perPage() + $index + 1 }}
                                </td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm capitalize">{{ $penginapan->nama }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $penginapan->jenis }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm capitalize">
                                    {{ $penginapan->nama_pengelola }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $penginapan->lokasi }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $penginapan->no_hp }}</td>
                                <td class="px-2 lg:px-4 py-2 text-xs lg:text-sm">{{ $penginapan->user->name ?? '-' }}</td>
                                <td class="px-2 lg:px-4 py-2">
                                    <div class="flex flex-wrap gap-1 lg:gap-2 justify-center">
                                        <a href="{{ route('super-admin.data.penginapan.show', $penginapan->id) }}"
                                        ...
                                        <a href="{{ route('super-admin.data.penginapan.edit', $penginapan->id) }}"
                                        ...
                                        <form action="{{ route('super-admin.data.penginapan.destroy', $penginapan->id) }}"
